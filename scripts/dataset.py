...
class LIDC(Dataset):
    def __init__(self, rater=4, data_dir = '/datadrive/raghav/lidc/lidcSeg/raw/', transform=None):
        ...
        self.data, self.targets = torch.load(os.path.join(self.data_dir, 'lidcSeg.pt'))
...
class LIDCPatientSplit(Dataset):
    def __init__(self, rater=4, data_list=None, transform=None):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        image, label = data_dict['image'], data_dict['masks']
        if self.transform is not None:
            image = self.transform(image)
